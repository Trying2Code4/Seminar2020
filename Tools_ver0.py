...
# ['USERID', 'MAILID', 'OFFERID', 'CLICK', 'MailOffer',
#   'USERID_ind', 'OFFERID_ind', 'ratioU', 'ratioO', 'res', 'val']
# Ouput:
# formatted: The recoded data in sparse format of [user, item, click]
def encoder2(data):
    ...
    formatted["USERID_ind"]=formatted["USERID_ind"]-1
    formatted["OFFERID_ind"]=formatted["OFFERID_ind"]-1
    formatted.columns = ['user', 'item', 'click']
...
def test_predictions3(p, trainset, testset, replacement = 0.02192184495444662):
    t0 = time.time()

    results=testset[["USERID","MAILID","OFFERID",'CLICK', 'MailOffer',
       'USERID_ind', 'OFFERID_ind', 'ratioU', 'ratioO']]
    # Get predictions
    results["PROBABILITY"]=results.apply(lambda row: get_test_pred(row['USERID_ind'],row['OFFERID_ind'],p,replacement), axis=1)
    print("Overriding some  predictions..")
...
def train_test(nObs=100000,testSize=0.2,seed=1):
    ...
    trainset, testset = train_test_split(observations, test_size = 0.2, random_state=1)
